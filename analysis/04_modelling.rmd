...
```{r}
anova(tsh_glmer_M_paper_soph_RI, tsh_glmer_M_no_test)
```
...
```{r}
summary(tsh_glmer_M_paper_soph_RI)
```

## Pairwise comparison

```{r}
summary(glht(tsh_glmer_M_paper_soph_RI, linfct = mcp(Thyroid_test = "Tukey")))
```
...
```{r}
TSH_DA  <- TSH_long %>% filter(min_size_max_method)
TSH_DA
```

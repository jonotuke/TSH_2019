...
# TSH  <- read_excel("../raw data/20191107_excel TSH.xlsx")
# TSH  <- read_excel("../raw data/20191205_excel TSH.xlsx")
# TSH  <- read_excel("../raw data/20191205_excel TSH V2.xlsx")
TSH  <- read_excel("../raw data/20191206_excel TSHV2.xlsx")
TSH
```

New column - Column X called `...26` by R. 

```{r}
TSH$...26[1:5]
```

Seems to be nothiing so remove
...
Nigel has mentioned that left blank rows between the papers

```{r}
TSH %>% filter(is.na(Paper))
TSH
TSH  <- 
  TSH %>% 
  filter(!is.na(Paper))
...
## Condition and restrictions on cohorts

This is no longer needed so ignore
...
Mentioned by NB that some like `other` will appear in multiple systems
...
Convert `free` to `Free`

```{r}
TSH  <- 
  TSH %>% 
  mutate(
    T3_total = str_to_title(T3_total)
  )
```
...
```{r}
write_rds(TSH, glue("../data/20191206-01_cleaning-data.rds"))
```

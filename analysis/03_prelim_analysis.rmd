...
```{r}
TSH_long  <- read_rds("../data/20190711-02_convert_long.rds")
TSH_long
```
...
So we will split T3 into T3_total amnd T3_free
...
```{r}
TSH_long_noT3  <- TSH_long %>% filter(Thyroid_test_4 != "T3")
```

```{r}
tsh_glmer  <- glmer(Sig ~ Thyroid_test_4 + (1 | Paper), data = TSH_long_noT3, family = binomial)
tsh_glm  <- glm(Sig ~ Thyroid_test_4, data = TSH_long_noT3, family = binomial)
anova(tsh_glmer, tsh_glm)
tsh_glmer_null  <- glmer(Sig ~ 1 + (1 | Paper), data = TSH_long_noT3, family = binomial)
anova(tsh_glmer, tsh_glmer_null)
summary(tsh_glmer)
```
